...
function isIdInvalid(id: string): boolean {
  for (let seqLen = 1; seqLen <= Math.floor(id.length / 2); seqLen++) {
    if (id.length % seqLen !== 0) continue;

    const numOfSeqOccurances = id.length / seqLen;
    const sequence = id.slice(0, seqLen);
    const repeatedSequence = Array(numOfSeqOccurances).fill(sequence).join("");
    if (id === repeatedSequence) return true;
  }
